...
class Orchestrator:
    def __init__(
        ...
        *,
        ...
        # Shared with the LiveKit agent (continuity.py at the repo root), so
        # both runtimes classify a mid-tool utterance the same way.
        self._router = UtteranceRouter()
    ...
    async def handle_user_message(self, text: str, source: str = "user") -> Request | None:
        ...
        if state.status in _BUSY:
            # Not every utterance during tool work means "throw it away".
            # "Are you still there?" is not a new request, and cancelling a
            # search that is 3 seconds into a 4-second call to answer it is
            # the wrong response — so is ignoring the user. Route first;
            # only a genuinely new request supersedes.
            #
            # This also keeps the interrupt path off the network: the fast
            # path answers without the analyze_intent round-trip below.
            if await self._handle_continuity_utterance(cleaned):
                return None
    ...
    async def _handle_continuity_utterance(self, text: str) -> bool:
        """Fast path for utterances that should NOT supersede in-flight work.

        Returns True when the utterance was fully handled here, meaning the
        current request survives untouched and no LLM call was made. Returns
        False to fall through to the normal supersede-and-reanalyse path,
        which is also the deliberate default for anything unrecognised: the
        worst case then is a search discarded and re-run, never a stale
        result spoken.

        CONSTRAINT deliberately falls through too. In this runtime a
        refinement ("under 5000") has to change the tool's parameters, which
        means re-running it with merged context — that is what
        ContextManager.merge and the follow-up path already do correctly.
        """
        current = await self.requests.get_current_request()
        if current is None:
            return False

        classification = self._router.classify(text, work_pending=True)
        intent = classification.intent

        if intent is Intent.AFFIRMATION:
            # A backchannel is not a request. Saying anything here would talk
            # over the user for no reason.
            await self._emit(
                EventType.USER_TURN,
                request_id=current.request_id,
                generation_id=current.generation_id,
                payload={"text": text, "continuity_intent": intent.value, "work_kept": True},
            )
            return True
        ...
        if intent is Intent.CANCEL:
            ...
            # just cancelled the request, not to the cancelled request, so
            # fencing would (correctly) discard it.
            await self._emit(
                EventType.ASSISTANT_RESPONSE_READY,
                payload={"text": cancel_sentence_for(description), "kind": "cancel_ack"},
            )
            try:
                await self.tts.speak(
                    cancel_sentence_for(description),
                    request_id=current.request_id,
                    generation_id=current.generation_id,
                    conversation_id=self.conversation_id,
                )
            except Exception as exc:
                await self._emit(EventType.ERROR, payload={"error": "tts_failure", "detail": str(exc)})
            return True

        return False

    async def _speak_without_disturbing_work(self, text: str, request: Request, kind: str) -> None:
        """Speak a short continuity line while the request keeps running.

        _speak() sets the conversation to SPEAKING, which is right for a final
        answer and wrong here — the tool is still EXECUTING and the state
        machine must not be told otherwise. So the status is restored
        afterwards, and only if this request is still the current one.
        """
        previous_status = (await self.store.snapshot()).status
        await self._speak(text, request, kind=kind)
        decision = await self.validator.validate(request.request_id, request.generation_id)
        if decision.accepted:
            await self.store.set_status(previous_status)
